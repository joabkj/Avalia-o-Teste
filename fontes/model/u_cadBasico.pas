...
implementation
  uses u_dicionarioDados;

{$R *.dfm}

procedure TcadBasico.AbrirTabela;
begin
  if not qy_dados.Active then
  begin
    qy_Dados.Close;
    qy_Dados.SQL.Clear;
    qy_Dados.SQL.Add('SELECT * FROM '+ TABELA);
    qy_Dados.open;
  end;
end;
...
begin
  ...
  if cb_tipo.ItemIndex = 0 then wl_campoDescr:= Order;
  if cb_tipo.ItemIndex = 1 then wl_campoDescr:= ID;
  ...
  begin
    try
      qy_Dados.Close;
      qy_Dados.SQL.Clear;
      qy_Dados.SQL.Add('SELECT * FROM '+ Tabela +' where '+ID +' = '+ID+ pesq_complementar+ wl_order);
      qy_Dados.open;
  ...
  begin
    ...
    try
      qy_Dados.Close;
      qy_Dados.SQL.Clear;
      qy_Dados.SQL.Add('SELECT * FROM '+ Tabela + wl_tipo + pesq_complementar+ wl_order);
      qy_Dados.open;
      if not qy_Dados.IsEmpty then
      gdr_principal.SetFocus;
    except
